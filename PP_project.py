...
def _sigmoid(x):
    ...
    y = 1.0 / (1 + np.exp(-x))
    return y
# Defining the mean square error function.
def _mse(target, output):
    return np.average((target - output) ** 2)

#Implementing the exception handling.
try:
    df=pd.read_csv("C:\\Users\hp\Downloads\Sonar221.csv")  #Reading the csv file.
    y1=df.loc[:,'label']
    y2=[]
    for i in y1:
        l=[]
    ...
    #creating the neural network with two layers where n depends on number of features in the dataset.
    network=[n,5,1]
    
    
    # Randomly assigning the weights and bias to the neural network.
    weights = []
    bias=[]
    ...
    for i in range(epochs):
        ...
        for j, input in enumerate(x):
            ...
            # feedforwarding our neural network with our iniatially defined random values of weights and bias.    
            y1 = y[j]
            ...
            # Backpropagating the error for the first forward pass.
            #Derivating the loss function w.r.t. weights and bias.
            ...
            #Updating the weights and bias according to the values of derivatives we have got.
    ...
    #plot between the mean square error and the number of epochs.
    plt.plot(e,L)
    plt.show()    
    ...
    # calculating our Accuracy for the tested data.   
